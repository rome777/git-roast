...
- **회원 탈퇴와 재가입**: 상단 내비게이션의 계정 버튼(로그아웃 버튼 왼쪽) → `/account` 계정 설정 페이지에서 직접 탈퇴할 수 있습니다. 계정과 분석 기록이 **모두 영구 삭제**되며(공유했던 카드 링크도 함께 사라집니다), **같은 이메일로 언제든 다시 가입**할 수 있습니다. 실행 전에 비밀번호를 다시 확인하고 이메일 주소를 직접 입력하게 해, 세션만 가지고는 계정을 지울 수 없도록 했습니다.
