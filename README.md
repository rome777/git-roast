...
- **비밀번호 정책 & 유출 검사**: 가입 시 8자 이상 + 숫자 + 특수문자를 요구하고, [Have I Been Pwned](https://haveibeenpwned.com/Passwords) 의 유출 목록과 대조해 이미 털린 비밀번호를 거부합니다. k-익명성 방식이라 **비밀번호 원문이나 전체 해시는 외부로 나가지 않습니다** (SHA-1 앞 5자리만 조회). 이 규칙은 새로 정하는 비밀번호에만 적용되므로 기존 회원의 로그인은 막지 않습니다.
- **이메일 확인**: 메일 발송이 설정된 배포에서는 가입 후 메일의 링크를 눌러야 로그인할 수 있습니다. 확인 전에는 세션이 발급되지 않습니다.
- **자동 가입 방지 (CAPTCHA)**: 로그인·회원가입에 Cloudflare Turnstile 을 적용할 수 있습니다. 키가 설정된 배포에서만 켜집니다.
- **비밀번호 변경**: `/account` 에서 바꿉니다. **현재 비밀번호를 다시 받습니다** — 세션만으로 바꿀 수 있으면 탈취된 쿠키 하나로 계정을 빼앗기기 때문입니다. 새 비밀번호는 가입과 똑같은 정책·유출 검사를 지나고, 시도 횟수도 로그인과 같은 한도로 제한됩니다.
- **닉네임**: 화면에 표시되는 이름입니다. 가입 시 임의로 정해지므로 손대지 않아도 메일 주소가 노출되지 않으며, `/account` 에서 언제든 바꿉니다. 운영진을 사칭하는 이름(`관리자`, `admin` 등)은 거부됩니다. **다른 사람과 겹쳐도 그대로 쓸 수 있습니다** — 계정을 구분하는 것은 이메일이고 닉네임은 표시용이라 중복을 막지 않습니다.
- **회원 탈퇴와 재가입**: 상단 내비게이션의 계정 칩(로그아웃 버튼 왼쪽) → `/account` 계정 설정 페이지에서 직접 탈퇴할 수 있습니다. 계정과 분석 기록이 **모두 영구 삭제**되며(공유했던 카드 링크도 함께 사라집니다), **같은 이메일로 언제든 다시 가입**할 수 있습니다. 실행 전에 비밀번호를 다시 확인하고 이메일 주소를 직접 입력하게 해, 세션만 가지고는 계정을 지울 수 없도록 했습니다.
