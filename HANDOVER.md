...
> **최종 갱신 시각**: 2026-09-08 23:50:00 (KST)  
...
| **일반 사용자 (USER)** | `/signup` 에서 가입한 이메일 | 가입 시 정한 값 (8자 이상 + 숫자 + 특수문자, 유출 목록 대조) | 본인 저장 내역만 격리 표출. 메일 발송이 설정된 배포에서는 **이메일 확인 후에야 로그인된다** |
...
│   │       ├── signup/route.ts         # 가입 (중복 409, 규칙 위반·유출 비밀번호 400)
│   │       ├── verify-email/route.ts   # 🆕 확인 메일 링크 도착점 → /login 으로 303
│   │       ├── resend-verification/route.ts  # 🆕 확인 메일 재발송 (계정 존재 미노출)
│   │       ├── delete-account/route.ts # 🆕 회원 탈퇴 (비밀번호 재확인, 마지막 관리자 차단)
...
│   ├── auth/
│   │   ├── CaptchaWidget.tsx           # 🆕 Turnstile 위젯 (사이트 키 없으면 렌더 안 함)
│   │   └── DeleteAccountSection.tsx    # 🆕 회원 탈퇴 (대시보드 하단, 접힘)
│   ├── ui/
│   │   └── Spinner.tsx                 # 🆕 로딩 표시 공용 부품 (Spinner/LoadingScreen/FormSkeleton)
│   └── layout/
...
│   ├── site-url.ts                     # 🆕 기준 절대 주소 (OG 태그와 메일 링크 공용)
│   ├── auth/                           # 인증 레이어
│   │   ├── password.ts                 # scrypt 해싱/검증, 레거시 자리표시자 판별
│   │   ├── password-rules.ts           # 🆕 복잡도 규칙 — 브라우저·서버 공용 (node:crypto 없음)
│   │   ├── pwned.ts                    # 🆕 HIBP k-익명성 유출 검사 (fail-open)
│   │   ├── captcha.ts                  # 🆕 Turnstile 서버 검증 (fail-closed)
│   │   ├── mailer.ts                   # 🆕 Resend REST 발송 (미설정 시 개발 콘솔 대체)
│   │   ├── email-verification.ts       # 🆕 HMAC 서명 확인 토큰 (DB 저장 없음, 24시간)
...
├── scripts/
│   ├── verify-auth.mjs                 # 인증·권한 회귀 검증 (npm run verify:auth, 57건)
...
2. **`components/layout/Navbar.tsx`**: 로그인 상태일 때 표시되는 **프로필 칩 자체**(이메일 앞부분 + 상태 점, 로그아웃 버튼 왼쪽)를 `/account` 링크로 만듦.
   - 처음에는 칩 옆에 톱니(⚙) 버튼을 따로 뒀지만, 이미 계정을 뜻하는 칩이 바로 옆에 있어 버튼이 둘로 늘어나는 것이 군더더기였다. 칩을 누르는 것이 자연스럽다.
...
### 2026-09-08 — 닉네임 도입, 비밀번호 변경 기능 추가

**① `users.nickname` 컬럼 추가 (`lib/db/database.ts`)**

- 두 엔진 모두 `NULL` 허용으로 붙였다. 기존 행에 넣을 값이 사람마다 달라야 해서
  (임의 생성) 한 번의 `ALTER ... DEFAULT` 로 채울 수 없다. 컬럼을 먼저 붙이고
  행마다 `UPDATE` 한다. 컬럼이 없던 순간에만 한 번 돈다 (`email_verified` 와 같은 방식).
- 로컬 SQLite 에서 실측: 기존 계정 4건에 임의 닉네임이 들어갔다 (2026-09-08).
- **UNIQUE 를 걸지 않았다.** 계정의 신원은 이메일이고 닉네임은 표시용이다.
  유일성을 요구하면 임의 생성이 충돌할 때 **가입 자체가 실패한다.**
- 이메일 아이디를 초기값으로 쓰지 않는다. 그렇게 두면 사용자가 손대기 전까지
  메일 주소 앞부분이 화면에 그대로 드러난다.

**② 임의 닉네임 생성 (`lib/auth/nickname.ts`)**

- 수식어 16개 × 동물 12개 × 두 자리 수 → "졸린 다람쥐38" 형태. `randomInt` 사용.
- 가입 폼에서 닉네임을 받지 않는다. 가입 단계를 늘리지 않으려는 것이고,
  비워 두면 화면마다 "닉네임 없음" 을 따로 처리해야 한다.

**③ 규칙은 클라이언트·서버 공용 (`lib/auth/nickname-rules.ts`)**

- `password-rules.ts` 와 같은 이유로 순수 함수만 분리했다. `node:crypto` 를 쓰는
  모듈에 두면 클라이언트 폼이 규칙을 베껴 적게 되고 한쪽이 곧 낡는다.
- 2~20자, 한글·영문·숫자·공백·`-`·`_`. `@` 가 빠져 이메일은 자동으로 걸린다.
- 운영진 사칭 금지어(`admin`/`관리자`/`gitroast` 등)는 공백·기호를 지운 뒤 비교하므로
  "관 리 자" 같은 우회도 막힌다 (실측 확인).

**④ 비밀번호 변경 (`app/api/auth/password/route.ts`)**

- **현재 비밀번호를 다시 받는다.** 세션만으로 바꿀 수 있으면 탈취된 쿠키 하나로
  공격자가 비밀번호를 바꿔 주인을 잠글 수 있다. 탈퇴와 같은 급으로 다룬다.
- 요청량 제한은 `"login"` 액션을 그대로 쓴다 — 현재 비밀번호를 받으므로
  로그인과 똑같은 추측 통로다 (탈퇴 라우트와 같은 판단).
- 네트워크를 타지 않는 검사(정책·현재값과 동일 여부)를 먼저 끝낸 뒤에 HIBP 를 부른다.
- DB 함수는 기존 `setUserPasswordHash` 를 재사용했다 (주석에 호출 경로 2개 명시).

**⑤ 알려진 함정 — 비밀번호를 바꿔도 다른 기기의 세션은 끊기지 않는다**

- 세션 토큰이 비밀번호와 무관하게 `SESSION_SECRET` 으로만 서명되므로, 변경 후에도
  기존 쿠키는 만료(7일)까지 유효하다. "쿠키가 털린 것 같아서 비밀번호를 바꾼다" 는
  경우에 기대와 다르게 동작한다.
- 끊으려면 `users.session_version` 같은 컬럼을 만들어 토큰에 실어야 한다.
  이번 작업에서는 하지 않았다 — 세션 설계를 건드리는 일이라 범위를 넘는다.

**⑥ 화면**

- `components/auth/NicknameSection.tsx`, `PasswordChangeSection.tsx` 신설 →
  `app/account/page.tsx` 가 닉네임 / 비밀번호 / 탈퇴 세 섹션을 담는다.
- 내비게이션 칩이 이메일 아이디 대신 닉네임을 보여 준다. 닉네임 저장 후
  `gitroast-auth-change` 이벤트를 쏴서 상단 이름표를 즉시 갱신한다 — 알려 주지 않으면
  옛 이름이 남아 "저장이 안 됐다" 로 읽힌다.
- 닉네임은 **세션 토큰에 넣지 않았다.** 토큰 수명이 7일이라 넣으면 변경이
  재로그인 전까지 반영되지 않는다. 화면은 `/api/auth/me` 로 DB 값을 읽는다.

---

### 2026-09-08 — 임의 닉네임의 꼬리번호 제거 (위 ② 정정)

위 항목의 "두 자리 수 → `졸린 다람쥐38` 형태" 는 더 이상 맞지 않다.
지금은 `졸린 다람쥐` 처럼 **숫자 없이** 만든다.

**왜 뺐나** — UNIQUE 를 걸지 않기로 한 판단과 숫자가 서로 모순이었다.
숫자가 하는 일은 기계적으로 충돌 회피인데, 유일성을 **보장하지는 못한다.**
즉 "중복은 상관없다" 고 해 놓고 중복을 신경 쓰는 척만 하는 절충이었다.
입장을 코드에서 분명히 하는 쪽을 골랐다.

**대가로 받아들인 것** — 조합이 16 × 12 = 192가지뿐이라 **동명이인이 자주 생긴다.**
회원 20명이면 겹치는 쌍이 있을 확률이 65% 정도다. 지금은 무해하다:
닉네임이 노출되는 곳은 본인의 상단 칩과 본인의 `/account` 뿐이고
(공유 카드 `/result/[id]` 는 작성자를 표시하지 않으며, 관리자 콘솔은 이메일로 나열한다),
모든 조회·삭제·권한 판정이 이메일이나 ID 를 키로 쓴다.

**언제 다시 봐야 하나** — 순위표·작성자 표기처럼 **닉네임을 남에게 보여 주는 화면을
만드는 순간.** 그때는 이미 중복이 쌓인 DB 를 마주하게 되므로,
UNIQUE 제약 + 충돌 시 재생성을 함께 넣어야 한다. 숫자만 되살리는 것은
같은 절충을 반복하는 것이니 하지 말 것.

참고: `supabase/schema.sql` 과 `docs/TECH_SPEC.md` 의 `profiles.nickname` 은
쓰이지 않는 레거시 스키마다(앱은 `lib/db/database.ts` 의 pg/SQLite 경로만 쓴다).
거기도 UNIQUE 는 없다.
...
### 2026-09-08 — 노션 8대 보안 수칙 대비 감사 (Supabase·RLS 항목 제외)

사용자 확인: **Supabase 를 쓰지 않고 RLS 대신 서버측 게이트로 대체하는 것은 승인됨.**
따라서 1번의 anon key 항목과 5번(RLS/Security Advisor)은 대상에서 제외하고 나머지를 대조했다.
기준 문서는 `docs/TECH_SPEC.md` 4장.

| # | 수칙 | 판정 | 근거 |
| :-- | :--- | :--- | :--- |
| 1 | 키·토큰 서버 전용 보관 | **충족** | 빌드 산출물(`.next/static`)에서 `GEMINI_API_KEY`·`SESSION_SECRET` 조각 검색 → 0건. `NEXT_PUBLIC_` 접두사가 붙은 비밀값 없음 |
| 2 | 이메일 확인 + SMTP | ~~미구현~~ → **구현됨** | 2026-09-08 후속 작업. 아래 "가입 방어 3종" 항목 참조 |
| 3 | 비밀번호 정책·유출 검사 | ~~부분~~ → **충족** | 2026-09-08 후속 작업. 복잡도 + HaveIBeenPwned 연동 |
| 4 | 남용 방어 (Rate Limit) | **미충족 → 조치함** | 아래 상세 |
| 5 | RLS & Security Advisor | 제외 | 서버측 게이트로 대체 승인 |
| 6 | Site URL·리다이렉트 화이트리스트 | **해당 없음/충족** | OAuth 리다이렉트가 없다. 외부 입력 기반 리다이렉트를 코드 전수 검색 → 0건 |
| 7 | 민감정보 미수집 + 백업 | **부분(확인 완료)** | 수집은 이메일 + scrypt 해시뿐, IP 도 HMAC 해시로만 저장 → 충족. **자동 백업은 Neon 무료 플랜에서 불가능하다** — 확인 결과는 아래 "Neon 백업 범위" 항목 |
| 8 | 무료 티어 절전 대응 | **충족(플랫폼 교체로 해소)** | Supabase 7일 절전 문제를 피해 Neon 을 골랐다. Neon 은 5분 유휴 시 scale-to-zero 지만 자동 재개(~0.5초)라 health-check 핑이 필요 없다. 대신 **100 CU-hours/월** 상한이 새 제약이다 |

**4번 상세 — 로그인이 무제한으로 열려 있었다**

수칙은 "IP 및 계정당 1분당 최대 5회, 일일 최대 30회". 실제로는 **`/api/analyze` 에만** 제한이
있었고 인증 경로에는 전혀 없었다. 운영 URL 에서 `admin@gitroast.dev` 에 대해
**8자 이상 비밀번호로 10회 연속 시도했더니 전부 401 로 통과**했다 — 제한 없음이 실증됐다.

> 처음 6자 비밀번호로 시도했을 때 400 이 떠서 잠깐 막힌 것처럼 보였는데,
> 그건 길이 검증에 걸린 것이지 방어가 아니었다.

조치: `enforceAuthRateLimit` 신설.
...
계정별 버킷을 따로 두는 이유는, 분산 IP 로 한 계정을 노리면 IP 카운터만으로는 안 막히기 때문이다.
운영 검증: 6회째부터 429.

**감사 중 별건으로 발견된 인증 우회 경로**

`getUserByEmailFromDb` 가 PostgreSQL 에서 계정을 **못 찾으면 SQLite 로 흘러갔다.**
배포 번들에 옛 스냅샷이 있던 상태(직전 항목 참조)에서는 **운영 DB 에서 지운 계정이
낡은 비밀번호 해시로 다시 로그인**될 수 있었다. `requireAdmin` 도 이 함수로 role 을 읽으므로
관리자 권한까지 되살아난다. 노출 구간은 첫 배포(16:11)부터 `.vercelignore` 배포(18:10)까지다.

같은 패턴을 전 함수에서 검사했는데 **이 한 곳뿐**이었다
(`setUserPasswordHash`·`getEvaluationByIdFromDb`·`deleteEvaluationFromDb`·
`clearUserEvaluationsFromDb` 는 무조건 `return` 이 있어 안전).

이 버그는 앞선 커밋의 "운영 SQLite 차단"이 **회원가입 500** 으로 드러냈다.
차단이 없었다면 계속 조용히 잘못 동작했을 것이다.

**회귀 검사 자체가 거짓 통과하고 있었다**

요청량 제한을 붙이자 `verify:auth` 가 32 PASS 로 나왔는데, 실제로는 **429 를 "기대한 실패"로
세고 있었다.** 특히 "비밀번호 미설정 계정은 어떤 값으로도 로그인 불가" 항목이 그랬다 —
차단돼서 통과한 게 아니라 제한에 걸려서 통과한 것이다.

수정: 실행마다 클라이언트 IP 를 분리하고, 429 는 어떤 검사에서도 통과로 세지 않으며,
요청량 제한 자체를 `[8]` 섹션으로 검사한다. 현재 **33 PASS / 1 FAIL**
(남은 1건은 보류 결정된 admin 비밀번호 항목이라 정직한 실패다).

**남은 미구현 (판단 필요)** — 네 항목 모두 같은 날 후속 작업에서 처리했다. 아래 항목 참조.

- ~~**2번 이메일 확인**~~ → 구현
- ~~**3번 비밀번호 복잡도·유출 검사**~~ → 구현
- ~~**4번 CAPTCHA**~~ → 구현 (키를 넣으면 켜진다)
- ~~**7번 백업**~~ → 확인 완료 (무료 플랜으로는 불가능하다는 결론)
...
### 2026-09-08 — 가입 방어 3종 (보안 수칙 2·3·4번) 과 Neon 백업 범위 확인

앞 항목의 감사에서 "판단 필요"로 남겨 둔 네 가지를 모두 처리했다.
셋은 구현, 하나(백업)는 **확인 결과 무료 플랜으로는 불가능**하다는 결론이다.

#### 1) 비밀번호 복잡도 — 로그인 경로에는 절대 걸지 않는다

규칙은 `lib/auth/password-rules.ts` 한 곳에 있다: 8자 이상 + 문자 + 숫자 + 특수문자,
같은 문자 4회 반복 금지, 이메일 아이디를 그대로 넣기 금지.

**함수를 둘로 나눈 것이 핵심이다.**

| 함수 | 쓰는 곳 | 검사 |
| :--- | :--- | :--- |
| `validatePassword` | **로그인** | 길이만 |
| `validateNewPassword` | 가입 · `set-password` | 길이 + 복잡도 |

로그인에도 복잡도를 걸면 **규칙을 강화하는 순간 그 이전에 가입한 사용자가 전부 400 을
받고 자기 계정에서 잠긴다.** 실제로 `rome777@gmail.com` 의 `admin1234` 는 특수문자가
없어 그대로 막혔을 것이다. 회귀 검사에 이 항목을 `[9]` 에 넣어 두었다 —
"복잡도 규칙이 기존 계정 로그인을 막지 않는다".

`lib/auth/password-rules.ts` 를 `password.ts` 에서 분리한 이유는 따로 있다.
`password.ts` 는 `node:crypto` 를 import 해서 **클라이언트 컴포넌트가 불러올 수 없다.**
그대로 두면 가입 폼이 규칙 문구를 따로 베껴 적게 되고, 두 벌이 되는 순간 한쪽이 낡는다.
지금은 가입 폼의 규칙 안내(`PASSWORD_RULES`)와 서버 검사가 같은 모듈을 쓴다.

> 예외: `scripts/set-password.mjs` 는 `.mjs` 라 TS 모듈을 불러올 수 없어 규칙이 두 벌이다.
> 파일에 그 사실을 명시해 두었다 — **한쪽을 고치면 다른 쪽도 고친다.**

#### 2) 유출 비밀번호 검사 — 원문을 보내지 않는다

`lib/auth/pwned.ts`. Have I Been Pwned 의 k-익명성 API 라 **SHA-1 해시의 앞 5자리만**
나가고, 돌아온 후보 목록에서 나머지 35자리를 우리 쪽에서 대조한다. `Add-Padding: true`
헤더로 응답 길이에서 내용을 추측하는 것까지 막는다.

**조회 실패는 통과시킨다(fail-open).** 여기서 막으면 HIBP 가 잠깐 죽는 순간 아무도
가입할 수 없다. 유출 검사는 복잡도 규칙 위에 얹는 추가 방어선이지 유일한 방어선이 아니다.
대신 조용히 넘어가지 않고 `[pwned]` 로그를 남긴다.
(요청량 제한의 `applyRules` 는 반대로 fail-closed 다 — 그쪽은 비용 방어가 목적이라
카운터가 죽었을 때 열어 두면 막으려던 상황이 그대로 벌어지기 때문이다.)

`DISABLE_PWNED_CHECK=true` 로 끌 수 있고, `PWNED_MAX_COUNT` 로 허용 횟수를 올릴 수 있다(기본 0).

#### 3) CAPTCHA (Cloudflare Turnstile) — 로그인과 가입 양쪽

`lib/auth/captcha.ts` + `components/auth/CaptchaWidget.tsx`.

**`NEXT_PUBLIC_TURNSTILE_SITE_KEY` 와 `TURNSTILE_SECRET_KEY` 가 둘 다 있을 때만 켜진다.**
한쪽만 있으면 검사를 건너뛰고 경고를 남긴다. 이유는 이 저장소가 이미 여러 번 데인
그 문제다 — 사이트 키는 `NEXT_PUBLIC_` 이라 **빌드 시점에 값이 박힌다.** 시크릿만
런타임에 넣고 재배포를 안 하면 위젯이 화면에 뜨지 않는데 서버는 토큰을 요구하는
상태가 되어 **가입이 통째로 막힌다.**

검증은 Cloudflare 가 공개한 테스트 키로 했다.

| 키 조합 | 요청 | 결과 |
| :--- | :--- | :--- |
| 항상 차단(`2x...`) | 토큰 없이 가입 | 400 "자동 가입 방지 확인을 먼저 완료해 주세요." |
| 항상 차단(`2x...`) | 더미 토큰으로 가입 | 400 "자동 가입 방지 확인에 실패했습니다." |
| 항상 차단(`2x...`) | 로그인 | 400 — 로그인도 같은 게이트를 지난다 |
| 항상 통과(`1x...`) | 더미 토큰으로 가입 | 201 |
| 항상 통과(`1x...`) | 브라우저에서 실제 폼 제출 | 위젯 "성공!" 표시 → 201 → 대시보드 진입 |

CAPTCHA 검증 실패는 **fail-closed** 다. 자동화 차단이 목적이라 검증이 죽었을 때
열어 두면 막으려던 상황이 그대로 벌어진다.

#### 4) 이메일 확인 — 토큰을 DB 에 저장하지 않는다

`lib/auth/email-verification.ts` + `lib/auth/mailer.ts`.
발송은 Resend **REST API** 로 한다(nodemailer 미사용). 서버리스 함수에서는 SMTP 포트로
나가는 장수명 커넥션이 잘 맞지 않고, 의존성도 늘지 않는다.

토큰은 세션 쿠키와 같은 방식으로 `SESSION_SECRET` 으로 HMAC 서명한 자기완결 토큰이다
(유효기간 24시간). 테이블·정리 작업·마이그레이션이 필요 없다. 서명 키에 용도 문자열
`email-verify:v1` 을 섞어 **세션 토큰과 확인 토큰을 서로 되쓸 수 없게** 했다.
1회용 보장은 토큰이 아니라 결과로 한다 — 확인이 끝나면 `users.email_verified` 가 true 가
되고, 같은 링크를 다시 눌러도 아무 일도 없다.

**`REQUIRE_EMAIL_VERIFICATION` 의 기본값은 `auto` 다(빈 값).**
메일을 보낼 수 없는 상태에서 강제하면 확인 링크를 받을 방법이 없어 **아무도 가입도
로그인도 못 한다.** 그래서 `RESEND_API_KEY` + `MAIL_FROM` 이 실제로 갖춰졌을 때만
자동으로 켜진다. `true`/`false` 로 못 박을 수도 있고, `true` 인데 메일 설정이 없으면
`npm run preflight` 가 **FAIL** 로 잡는다.

미확인 계정 판정은 **비밀번호가 맞은 뒤에** 한다. 앞에 두면 "이 계정은 미확인"이라는
응답 자체가 계정 존재 여부를 알려 준다. 재발송 엔드포인트도 가입 여부와 무관하게
**항상 같은 문구**를 돌려준다(회귀 검사 `[10]` 에서 확인).

`REQUIRE_EMAIL_VERIFICATION=true` 로 띄워 실제로 밟은 경로:

| 단계 | 결과 |
| :--- | :--- |
| 가입 | 201, **세션 쿠키 없음**, `verificationRequired: true` |
| 확인 전 로그인 | **403** `이메일 확인이 필요합니다` |
| 확인 링크 열기 | 303 → `/login?verify=ok&email=...` |
| 확인 후 로그인 | 200 + 세션 발급 |
| 기존 계정(`rome777@gmail.com`) | 200 — 소급 처리 덕에 영향 없음 |

#### 5) `users.email_verified` 를 뒤늦게 추가하면서 밟을 뻔한 지뢰 둘

**(a) 기존 사용자가 전부 잠긴다.** `ADD COLUMN ... DEFAULT FALSE` 만 하면 이미 쓰고 있던
계정이 모두 미확인이 된다. 그래서 **컬럼이 없던 DB 에 처음 붙이는 순간에만** 그 시점
이전에 만들어진 계정을 확인 완료로 소급 처리한다(`created_at < cutoff`).
기준 시각을 ALTER 전에 찍는 이유는, 서버리스에서 인스턴스 두 개가 동시에 초기화해도
그 사이 새로 가입한 계정까지 덩달아 통과되지 않게 하기 위해서다.

**(b) 위치 기반 INSERT 가 조용히 깨진다.** SQLite 시드가
`INSERT INTO users VALUES (?, ?, ?, ?, ?)` 였다. 컬럼이 하나 늘어나는 순간 개수가 어긋난다.
컬럼을 이름으로 지정하도록 고쳤다. **PostgreSQL 쪽은 원래 이름을 쓰고 있어 무사했다.**

운영자가 넣는 계정(`set-password`, 시드 관리자, 데모 계정)은 확인 완료로 만든다.
확인 메일을 받을 사람이 없는 계정이기 때문이다. `preflight` 는 확인이 켜진 배포에서
**관리자가 미확인이면 FAIL** 로 잡는다 — 그 상태면 `/admin` 에 아무도 못 들어간다.

#### 6) Neon 백업 범위 — 확인 결과 무료 플랜으로는 "일일 자동 백업"이 불가능하다

수칙 7번은 "PostgreSQL 일일 자동 백업 활성화"인데, **Neon 무료 플랜에는 그 기능이 없다.**

| 항목 | Free | Launch | Scale |
| :--- | :--- | :--- | :--- |
| 복원 창(history window) 기본값 | **6시간** | 1일 | 1일 |
| 복원 창 최대 | **6시간** (히스토리 1GB 상한) | 7일 | 30일 |
| 예약 백업(scheduled backups) | **없음** | 있음 | 있음 |
| 수동 스냅샷 | **1개** | 다수 | 다수 |
| 스토리지 | 0.5GB/프로젝트 | 무제한(종량) | 무제한(종량) |

**실질적 의미: 하루가 지난 실수는 되돌릴 수 없다.** 어제 잘못 지운 데이터는 6시간
창 밖이라 복구 경로가 없다. 무료 플랜의 스냅샷 1개도 수동이라 "떠 두는 것을 잊으면 없다".

지금 무엇을 해야 하는지는 **미결로 남긴다** (돈이 드는 선택이라 사용자 판단이 필요하다).
선택지는 셋이다.

1. **Launch 플랜으로 올린다** — 예약 백업 + 7일 복원 창. 가장 단순하다.
2. **외부에서 `pg_dump` 를 주기적으로 돌린다** — GitHub Actions cron 등. 무료지만
   덤프를 어디에 둘지(=개인 분석 기록이 든 파일을 어디에 쌓을지) 정해야 한다.
3. **아무것도 하지 않는다** — 지금 상태가 6시간 복원 창임을 알고 받아들인다.

`npm run preflight` 는 Neon 호스트를 발견하면 이 사실을 **매번 WARN 으로 다시 알린다.**
알고 넘어가는 것과 모르고 넘어가는 것은 다르다.

출처: [Neon plans](https://neon.com/docs/introduction/plans),
[History window](https://neon.com/docs/postgres/backup-restore/history-window)
(2026-09-08 확인).

#### 새로 생긴 환경 변수

전부 **비워 두면 해당 기능이 꺼진 채 서비스가 그대로 돈다.** 자세한 설명은 `.env.example` 12~14번.

| 변수 | 없으면 |
| :--- | :--- |
| `RESEND_API_KEY`, `MAIL_FROM` | 메일 발송 안 함 → 이메일 확인도 자동으로 꺼짐 |
| `REQUIRE_EMAIL_VERIFICATION` | `auto` (메일 설정이 있을 때만 강제) |
| `NEXT_PUBLIC_TURNSTILE_SITE_KEY`, `TURNSTILE_SECRET_KEY` | CAPTCHA 검사 건너뜀 |
| `DISABLE_PWNED_CHECK`, `PWNED_MAX_COUNT` | 유출 검사 켜짐, 허용 0회 |
| `RATE_LIMIT_RESEND_ACCOUNT_HOUR` | 재발송 대상 주소당 시간 3회 |

#### 회귀 검사

`[9] 가입 방어`, `[10] 이메일 확인 토큰` 두 절을 새로 넣었다. **43 PASS / 1 FAIL**
(이전 33 PASS / 1 FAIL). 남은 1건은 앞서 보류 결정된 `admin@gitroast.dev` 비밀번호
항목으로, 그대로 정직한 실패다.

검사에 쓰는 가입 비밀번호는 실행마다 새로 만든다(`freshPassword()`).
고정 문자열을 박아 두면 언젠가 유출 데이터셋에 들어가 검사가 통째로 빨개진다.
반대로 "유출 거부" 검사에는 **일부러** 유출된 값(`Password1!`, HIBP 58만여 건)을 쓴다.

**검사 자체가 관리자 계정의 요청량 버킷을 말려 죽이고 있었다**

새 검사를 붙이자 40 PASS / 4 FAIL 이 났는데, 실패 문구가 전부 `status=429` 였다.
원인은 코드가 아니라 검사였다.

`[8] 요청량 제한` 이 무차별 대입을 실증하려고 **`rome777@gmail.com` 을 10회 두드렸다.**
계정별 버킷은 시간당 20회인데, `[3]` 에서 이미 4회를 쓴 뒤였다. 그래서 그 뒤의
관리자 로그인이 429 가 되고 `adminCookie` 가 비면서 `[4] [5] [6]` 이 연쇄로 무너졌다.
**막힌 게 아니라 검사를 못 한 것이다** — 앞서 "429 를 기대한 실패로 세고 있었다" 와
같은 병의 반대 방향이다.

| 고친 곳 | 어떻게 |
| :--- | :--- |
| `[8]` 무차별 대입 대상 | 실계정 → **실재하지 않는 주소**. 제한은 DB 조회보다 앞에서 도므로 검증력은 같고, 실계정 버킷을 축내지 않는다 |
| `[9]` 복잡도-로그인 검사 | 실계정 로그인(200 기대) → **없는 주소에 규칙 위반 비밀번호**를 보내 `401`(400 이 아님)을 확인. 계정이 필요 없어졌다 |
| `[10]` 재발송 검사 | 양쪽 중 하나라도 429 면 통과로 세지 않고 "검사 불가" FAIL 로 명시 (재발송은 주소당 시간 3회) |

고친 뒤 **카운터를 초기화하지 않고 연속 3회** 돌려 43 PASS / 1 FAIL 이 그대로
재현되는 것을 확인했다. 이전에는 한 시간 안에 두 번 돌리면 무너졌다.

> 교훈: 검사가 **공유 자원(계정별 카운터)을 쓰면 절 사이에 숨은 의존이 생긴다.**
> 요청량 제한을 검증하는 절은 특히 그 자원을 일부러 소진시키므로, 다른 절이
> 쓰는 대상과 반드시 분리해야 한다.

---

### 2026-09-08 — 비동기 작업은 예외 없이 진행 중임을 표시한다 (작업 규칙)

사용자 지시로 정한 **규칙**이다. 이 저장소에서 네트워크를 타는 동작을 새로 붙일 때는
아래 세 가지를 함께 건다.

앞선 작업에서 붙인 가입 방어 3종이 계기였다. 가입 한 번에 CAPTCHA 검증 → 유출 목록
조회 → 계정 생성 → 메일 발송까지 외부 호출이 넷 붙어 수 초가 걸리는데, 버튼 문구는
여전히 "계정 생성 중..." 한 마디였다. 확인 메일 재발송·히스토리 전체 삭제·관리자
개별 삭제·로그아웃은 **아예 아무 반응이 없었다.**

> **눌렀는데 반응이 없으면 사용자는 안 눌렸다고 판단하고 다시 누른다.**
> 그 두 번째 클릭이 중복 가입이고 중복 삭제다. 표시의 진짜 목적은 안심시키는 것이
> 아니라 **중복 제출을 막는 것**이다.
...
| | 무엇을 | 왜 |
| :--- | :--- | :--- |
| **표시** | 스피너 + 지금 무엇을 하는지 적은 문구 | 멈춘 것과 일하는 중인 것을 구분시킨다 |
| **잠금** | `disabled` + `aria-busy` | 중복 제출을 실제로 막는다 |
| **자리** | 첫 진입은 빈 화면 대신 스켈레톤 | 응답이 온 뒤 내용이 튀어나오며 레이아웃이 밀리는 것을 막는다 |

공용 부품은 `components/ui/Spinner.tsx` 에 있다 — `Spinner` / `LoadingScreen` /
`FormSkeleton`. 새로 만들지 말고 이것을 쓴다.

#### 단계 문구는 서버가 실제로 밟는 순서대로

가입·로그인 버튼은 "처리 중" 으로 뭉뚱그리지 않고 서버 파이프라인 순서대로 문구를 넘긴다.

```
자동 가입 방지 확인 중... → 비밀번호가 유출된 적 있는지 대조 중...
→ 계정 만드는 중... → 확인 메일 보내는 중... → 대시보드로 이동 중...
```

**마지막 단계에서 멈추고 되돌아가지 않는다.** 메인 페이지의 분석 문구(`app/page.tsx`)
는 `% steps.length` 로 순환하는데, 그건 실제 소요가 길고 재미 요소라 그렇게 둔 것이다.
인증 폼처럼 짧고 진지한 경로에서 문구가 처음으로 되돌아가면 **가짜 진행률**로 읽힌다.

#### 놓치기 쉬운 함정 — 이동 중에 잠금을 풀면 안 된다

```ts
} finally {
  setLoading(false);   // ← 이러면 페이지가 넘어가기 직전 한순간 버튼이 되살아난다
}
```

`window.location.href = "/dashboard"` 는 이동을 **시작**만 하고 코드는 계속 흐른다.
`finally` 에서 무조건 잠금을 풀면 이동이 끝나기 전에 버튼이 원래대로 돌아가 다시
눌린다. `navigating` 플래그를 두고 이동 경로에서는 건너뛴다.

#### 손본 곳

| 위치 | 이전 | 이후 |
| :--- | :--- | :--- |
| `signup` · `login` 제출 | 문구 한 줄 | 스피너 + 단계 문구 + 이동 중 잠금 유지 |
| `signup` · `login` 확인 메일 재발송 | **무반응** | 스피너 + "보내는 중..." + 잠금 |
| `login` 로그아웃 / 데모 로그인 | **무반응** / 잠금 없음 | 스피너 + 잠금 |
| `login` · `signup` 첫 진입 | 폼이 먼저 깜빡임 | `FormSkeleton` |
| `Navbar` 세션 조회 | 빈칸 → 버튼이 튀어나옴 | 스켈레톤으로 자리 확보 |
| `Navbar` 로그아웃 | **무반응** | 아이콘이 스피너로 |
| `dashboard` 전체 비우기 | **무반응** (되돌릴 수 없는 동작인데) | 스피너 + "비우는 중..." |
| `admin` 개별 삭제 | **무반응**, 행이 갑자기 사라짐 | 해당 행 버튼만 스피너 + 행 흐리게 + 다른 삭제 잠금 |
| `admin` 첫 로딩 | 문구만 | 스피너 추가 |
| `EvaluationCard` PNG 저장 | 문구만 | 스피너 추가 |
| `ModeSuggestion` 재분석 | 문구만 | 아이콘이 스피너로 |
| `CaptchaWidget` 스크립트 로딩 | 빈 공간 | "불러오는 중" 자리표시 |

#### 접근성

표시는 **화면을 못 보는 사용자에게도 전달되어야 한다.** 진행 문구에
`role="status"` + `aria-live="polite"` 를 걸고, 아이콘만 바뀌는 버튼(내비게이션
로그아웃, 관리자 삭제)에는 `Spinner` 의 `label` 로 스크린 리더용 텍스트를 넣는다.

검증: 프로덕션 빌드에서 가입 버튼을 눌러 상태 전이를 표본 추출했다 —
`무료 회원가입 / disabled=false / aria-busy=false` → `입력값 확인 중... /
disabled=true / aria-busy=true / 스피너 있음`.

---

### 2026-09-08 — 회원 탈퇴와 재가입

사용자 요청. **탈퇴한 뒤 같은 이메일로 다시 가입할 수 있어야 한다**는 것이 요구사항이었고,
이 한 줄이 나머지 설계를 거의 다 결정했다.

#### 왜 하드 삭제인가

소프트 삭제(`deleted_at` 표시)를 쓰지 않았다. `users.email` 에 UNIQUE 제약이 걸려 있어
**행을 남겨 두면 같은 주소로 재가입할 수 없다.** 재가입을 허용하려면 결국
"삭제 표시된 행을 되살리기" 같은 우회로가 필요한데, 그건 지웠다고 해 놓고 이메일과
옛 비밀번호 해시를 계속 들고 있는 것이라 탈퇴의 의미에도 맞지 않는다.

**분석 기록도 함께 지운다.** `evaluations.user_email` 에 이메일이 그대로 들어 있어
계정만 지우면 개인정보가 남는다. 부작용으로 **그 사람이 공유한 카드 링크가 죽는다** —
그래서 실행 화면에 이 사실을 명시하고 동의를 받는다. "정말 삭제할까요?" 만 띄우고
링크가 죽는 걸 안 알리면 그건 동의를 받은 게 아니다.

PostgreSQL 에서는 evaluations → favorites → users 를 **한 트랜잭션**으로 묶는다.
중간에 실패해 기록만 지워지고 계정이 남으면 사용자는 "탈퇴했는데 로그인이 된다" 는
상태를 보게 된다. 이 함수는 **SQLite 로 폴백하지 않는다** — 탈퇴가 반쪽만 된 채
조용히 넘어가면 지워진 줄 알지만 데이터가 남는다.
...
| :-- | :--- | :--- |
| 1 | 로그인 세션 필수 | 기본 |
| 2 | **비밀번호 재확인** | 세션만으로 지우게 두면 자리 비운 브라우저나 탈취된 쿠키 하나로 계정이 통째로 날아간다. 탈퇴는 로그인보다 비싼 동작이다 |
| 3 | 로그인한 이메일을 직접 입력 | 되돌릴 수 없는 동작의 오클릭을 거른다 |
| 4 | 마지막 관리자 차단 (409) | 지우고 나면 `/admin` 에 아무도 못 들어간다 |

**지울 대상은 세션의 이메일이다.** 본문으로 받은 `confirmEmail` 은 확인용으로만 쓴다 —
본문을 신뢰해 대상을 정하면 남의 계정을 지정할 수 있다.

공용 **데모 계정은 탈퇴할 수 없다**(403). 여러 사람이 함께 쓰는 계정이라 한 사람이
지우면 다른 사람의 데모 기록까지 날아간다.

성공하면 세션 쿠키를 반드시 만료시킨다. 남겨 두면 없는 계정을 가리키는 쿠키가 떠돈다.

#### 화면

`components/auth/DeleteAccountSection.tsx` — 대시보드 맨 아래에 **접어서** 둔다.
자주 쓰는 동작이 아니고, 실수로 열려서는 안 되는 동작이다.
펼치기 → 무엇이 사라지는지 전부 읽기 → 이메일 직접 입력 + 비밀번호 → 실행.
두 입력이 맞기 전까지 실행 버튼은 잠겨 있다.

탈퇴 후에는 `/signup?left=1` 로 보내고 "같은 이메일로 다시 가입할 수 있습니다" 를 띄운다.
탈퇴한 사람이 마음을 바꿨을 때 길이 막혀 있으면 안 된다.

#### 검증

회귀 검사에 `[11] 회원 탈퇴 · 재가입` 을 추가했다. **56 PASS / 1 FAIL**
(이전 43 PASS / 1 FAIL, 남은 1건은 보류 결정된 admin 비밀번호 항목).

핵심 경로는 브라우저에서도 실제로 밟았다.

| 확인 | 결과 |
| :--- | :--- |
| 비로그인 / 비밀번호 누락 / 이메일 불일치 / 비밀번호 오류 | 401 / 400 / 400 / 401 |
| 실패한 시도 뒤 계정 상태 | 그대로 살아 있음 |
| 정상 탈퇴 | 200, 기록 1건 함께 삭제, 세션 쿠키 만료 |
| 탈퇴 후 옛 세션 · 옛 비밀번호 | 둘 다 통하지 않음 |
| **같은 이메일로 재가입** | 201, **새 id** 발급 |
| 재가입 계정의 보관함 | 비어 있음 (옛 기록이 새 계정으로 새지 않는다) |
| 데모 계정 탈퇴 | 403 |
| 마지막 관리자 탈퇴 | 409, 계정 살아 있음 |

> 마지막 관리자 검사는 로컬에서 실제 관리자 둘을 잠시 `user` 로 내려 tempadmin 을
> 유일한 관리자로 만든 뒤 확인하고, 같은 스크립트에서 원복했다.

UI 잠금도 확인했다 — 아무것도 입력 안 함/이메일 불일치/비밀번호 지움 세 경우 모두
실행 버튼이 잠기고, 이메일이 정확히 일치하고 비밀번호가 있을 때만 풀린다.
