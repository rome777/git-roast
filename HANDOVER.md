...
| 2 | 이메일 확인 + SMTP | **미구현** | `/api/auth/signup` 이 형식 검사만 하고 즉시 계정 생성 + 세션 발급. 아무 이메일로나 가입된다 |
| 3 | 비밀번호 정책·유출 검사 | **부분** | 최소 8자만 검사(`validatePassword`). 복잡도 요구 없음, HaveIBeenPwned 연동 없음 |
...
| 7 | 민감정보 미수집 + 백업 | **부분** | 수집은 이메일 + scrypt 해시뿐, IP 도 HMAC 해시로만 저장 → 충족. **자동 백업은 미확인** — Neon 무료 플랜의 복원 범위를 확인해야 한다 |
...
- **2번 이메일 확인**: 미구현. 가입 스팸과 타인 이메일 도용이 가능하다.
  구현하려면 메일 발송 경로(Resend 등)가 필요하다.
- **3번 비밀번호 복잡도·유출 검사**: 최소 8자만. HaveIBeenPwned 는 k-anonymity API 라
